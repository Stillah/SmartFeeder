...
class TimeManagmentInterface(Protocol):
    ...
    @abstractmethod
    def add(self, user_id: UUID, start: int, end: int) -> None:
        """Add a time interval for disposing food for a user."""

    @abstractmethod
    def load_all(self, user_id: UUID) -> list[tuple[int, int]]:
        """Load all time intervals for a user."""

    @abstractmethod
    def update(self, user_id: UUID, old_start: int, old_end: int, new_start: int | None, new_end: int | None) -> None:
        """
    ...
    @abstractmethod
    def delete(self, user_id: UUID, start: int, end: int) -> None:
