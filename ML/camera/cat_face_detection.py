...
def start_camera(user_id: str, display: bool=False) -> None:
    ...
    cat_face_cascade = cv2.CascadeClassifier('haarcascade/haarcascade_frontalcatface_extended.xml')  # Use custom path for cat face cascade
    smile_cascade = cv2.CascadeClassifier('haarcascade/haarcascade_smile.xml')  # Use custom path for smile cascade
    ...
    while True:
        ...
        if len(cat_faces) > 0 and (not last or datetime.now() - last > timedelta(seconds=2)):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-display', action='store_true', help='Display what camera sees')
    parser.add_argument('--user_id', type=str, default="123e4567-e89b-12d3-a456-426614174000", help='User UUID')
