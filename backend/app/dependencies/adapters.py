...
def get_pets_adapter(session: AsyncSession = Depends(get_db_session)) -> PetsAdapter:
    return PetsAdapter(session)

def get_feeder_status_adapter(session: AsyncSession = Depends(get_db_session)) -> FeederStatusAdapter:
    return FeederStatusAdapter(session)

def get_history_adapter(session: AsyncSession = Depends(get_db_session)) -> HistoryAdapter:
    return HistoryAdapter(session)

def get_image_adapter(session: AsyncSession = Depends(get_db_session)) -> ImageAdapter:
    return ImageAdapter(session)

def get_time_manager_adapter(session: AsyncSession = Depends(get_db_session)) -> TimeManagerAdapter:
