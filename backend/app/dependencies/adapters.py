...
def get_feeder_status_adapter(session: AsyncSession = Depends(get_session)) -> FeederStatusAdapter:
...
def get_time_manager_adapter(session: AsyncSession = Depends(get_session)) -> TimeManagerAdapter:
